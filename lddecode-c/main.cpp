...
int main(int argc, char *argv[])
{
    ...
    map<String,bool> extraBools;
    QString audioPipe = "None";

    extraBools.insert(pair<String,bool>("useAGC",!parser.isSet(noAGCOption)));
    extraBools.insert(pair<String,bool>("write_RF_TBC",parser.isSet(RFTBCOption)));
    extraBools.insert(pair<String,bool>("write_pre_efm",parser.isSet(preEFMOption)));
    extraBools.insert(pair<String,bool>("lowband",parser.isSet(lowbandOption)));
    extraBools.insert(pair<String,bool>("verboseVITS",parser.isSet(verboseVITSOption)));
    ...
    if (parser.isSet(palOption)) {
        ...
        extraBools.insert(pair<String,bool>("PAL_V4300D_NotchFilter",paser.isSet(v4300DOption)));
    }
    else {
        standard = "NTSC"

        extraBools.insert(pair<String,bool>("NTSC_ColorNotchFilter",parser.isSet(ntscColorNotchOption)));
    ...
    map<String,int> extraInts;

    if (parser.isSet(seekFrameOption)) {
        extraInts.insert(pair<String,int>("seek",paser.value(deempAdjustOption)));
    }
    ...
    map<String,float> extraFloats;

    if (parser.isSet(deempAdjustOption)) {
        extraFloats.insert(pair<String,float>("deemp_adjust",paser.value(deempAdjustOption)));
    }
    ...
    if (parser.isSet(startFilelocOption)) {
        extraFloats.insert(pair<String,float>("start_fileloc"),parser.value(startFilelocOption));
    }

    if (parser.isSet(mtfOption)) {
        extraFloats.insert(pair<String,float>("mtf_offset"),parser.value(mtfOption));
    }

    if (parser.isSet(mtfOffsetOption)) {
        extraFloats.insert(pair<String,float>("start_fileloc"),parser.value(mtfOffsetOption));
    }
